...
Test cases:
    * `test_build_in_type_validator` tests the validator with a variety
       of different rule types and data to validate that builtin types
       (e.g floats, ints, strings) are processed correctly
"""
...
class TestBuiltInTypeValidator(BaseValidatorTest):
    """Test cases for the Builtin Type Validator"""
    ...
    @parameterized.expand([
        ...
        ('int_type_mismatch',
            RuleType(schema_type=SchemaTypes.INT), 'hello', True),
        ('str_type_mismatch',
            RuleType(schema_type=SchemaTypes.STR), None, True),
        ('float_type_mismatch',
            RuleType(schema_type=SchemaTypes.FLOAT), 3, True),
        ('bool_type_match_true',
            RuleType(SchemaTypes.BOOL), True, False),
        ('bool_type_match_false',
            RuleType(SchemaTypes.BOOL), False, False),
        ('bool_type_mismatch',
            RuleType(SchemaTypes.BOOL), 'true', True),
        ('non_builtin_type',
            RuleType(SchemaTypes.REGEX, regex='^test'), 'test', False),
    ])
    def test_build_in_type_validator(self, name: str, rtype: RuleType,
                                     data: Data, expect_violations: bool):
